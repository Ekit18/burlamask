...
import { AllStaticFiles, ImageAwsService } from './parts-guides-aws.service';
...
export class ImageAwsController {
    ...
    async addPartImg(
        ...
        const urls: string[] = await Promise.all(
            files.map(
                (file) => this.ImageAwsService.addImage(file.buffer, file.originalname, imageData.description)
            )
        );
        return urls;
    }
